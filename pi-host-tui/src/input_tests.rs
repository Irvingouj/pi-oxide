...
fn build_app(input: &str, cursor_pos: usize) -> App {
    ...
    app.cursor_pos = cursor_pos.min(input.len());
    app
}
...
fn ctrl_a_moves_cursor_to_start() {
    ...
    assert_eq!(app.cursor_pos, 0, "Ctrl+A should move to line start");
...
fn ctrl_e_moves_cursor_to_end() {
    ...
    assert_eq!(app.cursor_pos, 11, "Ctrl+E should move to line end");
    assert_eq!(app.input, "hello world");
}
...
fn ctrl_b_moves_cursor_left() {
    ...
    assert_eq!(app.cursor_pos, 2, "Ctrl+B should move left one char");
}
...
fn ctrl_f_moves_cursor_right() {
    ...
    assert_eq!(app.cursor_pos, 3, "Ctrl+F should move right one char");
}
...
fn ctrl_w_deletes_word_backward() {
    ...
    assert_eq!(app.input, "hello ", "Ctrl+W should delete 'world'");
...
fn ctrl_w_consecutive_accumulates_in_kill_ring() {
    ...
    assert_eq!(app.kill_ring.peek(), Some("bar baz"));
}
...
fn ctrl_k_deletes_to_line_end() {
    ...
    assert_eq!(app.input, "hel", "Ctrl+K should delete from cursor to end");
    assert_eq!(app.cursor_pos, 3);
    assert_eq!(app.kill_ring.peek(), Some("lo world"));
}
...
fn ctrl_u_deletes_to_line_start() {
    ...
    assert_eq!(app.cursor_pos, 0);
    assert_eq!(app.kill_ring.peek(), Some("hello w"));
}
...
fn ctrl_d_deletes_char_forward() {
    ...
    assert_eq!(app.input, "helo", "Ctrl+D should delete char at cursor");
...
fn ctrl_y_yanks_from_kill_ring() {
    ...
    assert_eq!(app.input, "hello world", "Ctrl+Y should yank 'world'");
...
fn alt_y_yank_pop_noop_with_single_entry() {
    ...
    app.handle_key(ctrl(KeyCode::Char('y'))); // yank
    let after_yank = app.input.clone();
    ...
    assert_eq!(app.input, after_yank, "Yank-pop with single entry is no-op");
}
...
fn alt_backspace_deletes_word_backward() {
    ...
    assert_eq!(app.input, "hello ", "Alt+Backspace should delete 'world'");
    assert_eq!(app.cursor_pos, 6);
}
...
fn alt_delete_deletes_word_forward() {
    ...
    assert_eq!(app.input, "hello ", "Alt+Delete should delete 'world'");
    assert_eq!(app.cursor_pos, 6);
}
...
fn ctrl_left_moves_word_left() {
    ...
    assert_eq!(app.cursor_pos, 6, "Ctrl+Left should jump to 'w'");
}
...
fn ctrl_right_moves_word_right() {
    ...
    assert!(app.cursor_pos > 0, "Ctrl+Right should move forward by word");
    assert!(
        app.cursor_pos < 11,
...
fn shift_enter_inserts_newline() {
    ...
    assert_eq!(app.cursor_pos, 6); // after the newline
...
fn esc_dismisses_suggestions_first() {
    ...
    app.suggestions = vec!["/help".to_string()];
    app.handle_key(plain(KeyCode::Esc));
    assert!(!app.show_suggestions, "Esc should dismiss suggestions");
    assert!(
        !app.should_quit,
...
fn tab_cycles_suggestions() {
    ...
    assert!(app.show_suggestions, "Tab should cycle suggestions");
}
...
fn kill_ring_push_single_entry() {
    ...
    assert_eq!(app.kill_ring.peek(), Some("hello"));
}
...
fn kill_ring_rotate_with_two_entries() {
    ...
    assert_eq!(app.kill_ring.peek(), Some("bar"), "Most recent is 'bar'");
    ...
    assert_eq!(app.input, "foo baz ", "Yank-pop should rotate to 'baz'");
}
...
fn ctrl_d_deletes_multi_byte_char() {
    let mut app = build_app("日本", 0);
    app.handle_key(ctrl(KeyCode::Char('d')));
    assert_eq!(app.input, "本", "Ctrl+D should delete one multi-byte char");
    assert_eq!(app.cursor_pos, 0);
}
...
fn ctrl_w_deletes_multi_byte_word() {
    ...
    assert_eq!(app.input, "日本 ", "Ctrl+W should delete '語'");
    assert_eq!(app.cursor_pos, 7); // "日本 " = 3 + 3 + 1 = 7 bytes
}
...
fn ctrl_right_all_whitespace() {
    let mut app = build_app("   ", 0);
    app.handle_key(ctrl(KeyCode::Right));
    assert_eq!(app.cursor_pos, 3, "Ctrl+Right should skip all whitespace");
}
