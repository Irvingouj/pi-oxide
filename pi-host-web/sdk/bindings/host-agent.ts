...
export class HostAgent {
	...
	steer(message: WasmAgentMessage): { events: RawAgentEvent[] } {
		const result = unwrap(hostSteer(this.handle, message)) as {
	...
	getPersistData(): PersistData {
		const result = unwrap(getHostAgentPersistData(this.handle)) as {
			state: PersistData;
		};
		return result.state;
	}
...
export async function createHostAgentInstance(
	...
	let handle: number;
	if (sessionState) {
		logger.info("Restoring host agent from session state", {
			sessionId: config.sessionId,
		});
		const restored = unwrap(restoreHostAgent(options, sessionState)) as {
			handle: number;
		};
		...
		const result = unwrap(
			createHostAgent(options, buildContextBudget(config.context)),
		) as { handle: number };
