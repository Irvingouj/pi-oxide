...
import {
	CodingToolRegistry,
	MemoryWorkspace,
} from "../src/tools/codingTools.ts";
import { PI_CODING_TOOLS } from "../src/tools/schemas.ts";

const apiKey = process.env.ANTHROPIC_API_KEY;
if (!apiKey) {
	console.error(
		"SKIP: ANTHROPIC_API_KEY is not set. Set it to run the real LLM smoke test.",
	);
	process.exit(0);
}
...
async function main() {
	...
	try {
		...
		if (!toolNames.includes("edit") && !toolNames.includes("write")) {
			console.error(
				"❌ FAIL: trace does not include an edit or write tool call",
			);
			host.cleanup(result.handle);
			process.exit(1);
		}
