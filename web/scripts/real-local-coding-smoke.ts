...
if (!apiKey) {
	console.error(
		"SKIP: ANTHROPIC_API_KEY is not set. Set it to run the real local coding smoke test.",
	);
	process.exit(0);
}
...
async function main() {
	...
	const host = new PersistentHost(
		...
		llm,
		{
			...
			execute: () => ({
				error: {
					code: "sync_fallback_unavailable",
					message: "sync tools not available when using ToolRuntime",
				},
			}),
		},
		runtime,
	...
	try {
		...
		for (const entry of result.trace) {
			if (entry.phase === "action") {
				...
				if (entry.type === "tool_execution_update" && data) {
					const preview = String(data.chunk ?? "")
						.slice(0, 60)
						.replace(/\n/g, "\\n");
					console.log(
						`  [event]  ${entry.type} stream=${JSON.stringify(data.stream ?? "?")} seq=${JSON.stringify(data.sequence ?? "?")} "${preview}"`,
					);
				} else {
				...
				if (entry.type === "tool_done") {
					...
					const content = payload?.content as
						...
						| undefined;
					const preview =
						content?.[0]?.text?.slice(0, 120).replace(/\n/g, "\\n") ?? "";
					console.log(
						`  [host]   ${entry.type} (${data.tool_name as string})${isError ? " [ERROR]" : ""}${preview ? " -> " + preview : ""}`,
					);
				} else {
		...
		console.log(
			`Artifacts stored: ${artifacts["store"] instanceof Map ? artifacts["store"].size : "N/A"}`,
		);
		console.log();
		...
		if (result.terminalAction.type !== "finished") {
			console.error(
				`❌ FAIL: terminal action is ${result.terminalAction.type}, expected finished`,
			);
			failed = true;
		}
		...
		if (fs.existsSync(indexPath)) {
			...
			console.log(
				`index.html: ${size} bytes, has <html>: ${hasHtml}, has <body>: ${hasBody}, mentions Ottawa: ${hasOttawa}`,
			);
		...
		if (projectionLogs.length > 0) {
			console.log(
				`✅ Context projection ran (${projectionLogs.length} time(s))`,
			);
		} else {
		...
		if (toolExecUpdates.length > 0) {
			console.log(
				`✅ Streaming tool updates present (${toolExecUpdates.length} events)`,
			);
		} else {
		...
		if (toolExecStarts.length > 0) {
			console.log(
				`✅ Tool execution start events present (${toolExecStarts.length} events)`,
			);
		} else {
		...
		if (fs.existsSync(sessionFile)) {
			...
			const sessionEntries = sessionLines.map(
				(l) => JSON.parse(l) as { kind: string },
			);
			const sessionKinds = sessionEntries.map((e) => e.kind);
			if (!sessionKinds.includes("session_start")) {
				console.error("❌ FAIL: session missing session_start");
				failed = true;
			}
			if (!sessionKinds.includes("user_prompt")) {
				console.error("❌ FAIL: session missing user_prompt");
				failed = true;
			}
			if (!sessionKinds.includes("tool_result")) {
				console.error("❌ FAIL: session missing tool_result");
				failed = true;
			}
			if (!sessionKinds.includes("session_end")) {
				console.error("❌ FAIL: session missing session_end");
				failed = true;
			}
			if (
				sessionKinds.includes("session_start") &&
				sessionKinds.includes("session_end")
			) {
				console.log(
					`  Session kinds: ${[...new Set(sessionKinds)].join(", ")}`,
				);
			}
		} else {
		...
		try {
			...
			console.log(
				`✅ Session loaded: ${loaded.entries.length} entries, session_id=${loaded.metadata.session_id}`,
			);
		} catch (err) {
		...
		if (fs.existsSync(artifactsCheckDir)) {
			...
			console.log(
				`✅ Artifacts directory exists (${artifactFiles.length} files)`,
			);
		} else {
		...
		try {
			...
			execFileSync(
				"sh",
				["-c", "pkill -f 'python3 -m http.server 8421' 2>/dev/null || true"],
				{ timeout: 3000 },
			);
		} catch {
			// ignore
