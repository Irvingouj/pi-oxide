...
async function main() {
  ...
  page.on("console", (msg) => {
    ...
    if (text.includes("prompt called") || text.includes("feedLlmChunk") || text.includes("onLlmDone") || text.includes("continueTurn")) {
  ...
  const state2 = await page.evaluate(() => ({
    ...
    allText: Array.from(document.querySelectorAll(".msg")).map((el) => el.textContent?.slice(0, 80)),
  }));
  ...
  if (state2.errorMsgs > 0) {
    console.error("\nFAIL: Found", state2.errorMsgs, "error messages:", state2.allText);
    process.exit(1);
  }
