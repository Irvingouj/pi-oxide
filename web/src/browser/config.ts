...
export function initEnvDefaults(): void {
	...
	if (apiKey && !apiKey.value && viteEnv.VITE_API_KEY)
		apiKey.value = viteEnv.VITE_API_KEY;
	if (baseUrl && !baseUrl.value && viteEnv.VITE_BASE_URL)
		baseUrl.value = viteEnv.VITE_BASE_URL;
	if (model && !model.value && viteEnv.VITE_MODEL)
		model.value = viteEnv.VITE_MODEL;
}
