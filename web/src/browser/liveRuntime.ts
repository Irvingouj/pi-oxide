...
console.log = (...a: unknown[]) => { captureConsole("log", a); origConsole.log(...a); };
console.warn = (...a: unknown[]) => { captureConsole("warn", a); origConsole.warn(...a); };
console.error = (...a: unknown[]) => { captureConsole("error", a); origConsole.error(...a); };
console.info = (...a: unknown[]) => { captureConsole("info", a); origConsole.info(...a); };
...
export class LiveBrowserRuntime implements BrowserRuntime {
  getPage(): BrowserPageSnapshot {
    ...
    const focused =
      ae && ae !== document.body ? snapshotElement(ae, "") : null;
    return {
      url: location.href,
  ...
  click(selector: string): BrowserToolResult {
    ...
    if (!el) {
      return {
        ok: false,
        error: { code: "element_not_found", message: `No element matches: ${selector}` },
      };
    }
    el.click();
    return { ok: true };
