...
import { resolveLocalPath, LocalPathError } from "./path.ts";
...
export function handleLocalRead(args: Record<string, unknown>, cwd: string): object {
  if (typeof args.path !== "string") {
    return errorPayload("missing_path", "read requires a 'path' string argument");
  }
  ...
  if (stat.isDirectory()) {
    return errorPayload("is_directory", `path is a directory, not a file: ${args.path}`);
  }
  ...
  // Byte truncation
  let text = selected.map((line, i) => `${startLine + i + 1}: ${line}`).join("\n");
  ...
  const details = truncationInfo ? { truncated: true, ...truncationInfo } : undefined;
...
export function handleLocalWrite(args: Record<string, unknown>, cwd: string): object {
  if (typeof args.path !== "string") {
    return errorPayload("missing_path", "write requires a 'path' string argument");
  }
  if (typeof args.content !== "string") {
    return errorPayload("missing_content", "write requires a 'content' string argument");
  }
  ...
  try {
    ...
    return errorPayload("write_error", `failed to write file: ${args.path}: ${msg}`);
  }
...
export function handleLocalEdit(args: Record<string, unknown>, cwd: string): object {
  if (typeof args.path !== "string") {
    return errorPayload("missing_path", "edit requires a 'path' string argument");
  }
  if (!Array.isArray(args.edits) || args.edits.length === 0) {
    return errorPayload("missing_edits", "edit requires a non-empty 'edits' array argument");
  }
  ...
  try {
    ...
    return errorPayload("read_error", `failed to read file for editing: ${args.path}`);
  }

  // Validate edits against the original file
  const applied: Array<{ oldText: string; newText: string; occurrences: number }> = [];
  for (const edit of args.edits as Array<{ oldText?: unknown; newText?: unknown }>) {
    if (typeof edit.oldText !== "string") {
      return errorPayload("missing_oldText", "each edit must have an 'oldText' string");
    }
    if (edit.oldText.length === 0) {
      return errorPayload("empty_oldText", "edit oldText must not be empty");
    }
    if (typeof edit.newText !== "string") {
      return errorPayload("missing_newText", "each edit must have a 'newText' string");
    }
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    return errorPayload("write_error", `failed to write edited file: ${args.path}: ${msg}`);
  }

  const diffLines = applied.map(
    (e) => `  -${e.oldText}\n  +${e.newText}`,
  ).join("\n");

  return okPayload(
    `edited ${args.path}`,
    { edits: applied.length, diff: diffLines },
  );
}
