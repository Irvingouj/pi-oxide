...
import { RealAgentHost, RealLlm, type TraceEntry, type ContextProjectionConfig } from "../providers/realLlm.ts";
 ...
 */
function traceToSessionKind(phase: TraceEntry["phase"], type: string): SessionEntryKind | null {
  if (phase === "host") {
    switch (type) {
      case "prompt": return "user_prompt";
      case "tool_done": return "tool_result";
      case "llm_result": return "assistant_message";
      case "create_agent": return "lifecycle_event";
      case "destroy_agent": return "lifecycle_event";
      default: return null;
    }
  }
  if (phase === "event") {
    switch (type) {
      case "tool_execution_start": return "tool_call";
      case "tool_execution_update": return "tool_streaming_update";
      case "tool_execution_end": return "lifecycle_event";
      case "agent_start": return "lifecycle_event";
      case "agent_end": return "lifecycle_event";
      default: return null;
    }
  }
  return null;
...
export class PersistentHost {
  ...
  constructor(
    ...
    const config = (llm as unknown as { contextProjection?: ContextProjectionConfig }).contextProjection;
    if (config) {
      // Wrap the artifact store to also write to filesystem
      const originalStore = config.artifacts;
      config.artifacts = new DualArtifactStore(originalStore, this.fileArtifacts);
    }
  }
