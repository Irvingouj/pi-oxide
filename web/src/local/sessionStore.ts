...
export class LocalSessionStore {
	...
	constructor(
		sessionDir: string,
		metadata: Omit<SessionMetadata, "created_at" | "updated_at">,
	) {
	...
	append(
		kindOrEntry: SessionEntryKind | Omit<SessionEntry, "seq" | "timestamp">,
		data?: unknown,
	): void {
		const entry: SessionEntry =
			typeof kindOrEntry === "string"
	...
	for (let i = 0; i < lines.length; i++) {
		...
		try {
			...
			throw new SessionCorruptError(
				i + 1,
				`invalid JSON: ${lines[i].slice(0, 100)}`,
			);
		}
