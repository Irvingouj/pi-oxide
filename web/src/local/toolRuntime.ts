...
import {
	type StreamingBashHandle,
	startStreamingBash,
} from "./streamingBashTool.ts";
...
export class ToolRuntime {
	...
	private async executeBash(
		...
		if (typeof args.command !== "string") {
			return {
				error: {
					...
					message: "bash requires a 'command' string argument",
				},
			};
		}
		...
		if (this.bashPolicy.mode !== "unrestricted") {
			return {
				error: {
					...
					message: `bash is not allowed (policy mode: "${this.bashPolicy.mode}")`,
				},
			};
		}
		...
		const bashHandle = startStreamingBash(
			command,
			this.cwd,
			{
