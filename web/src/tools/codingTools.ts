...
import { validateWorkspacePath } from "./path.ts";
import { PI_TOOL_NAMES, TOOL_NAMES } from "./schemas.ts";
...
export class MemoryWorkspace {
	...
	listDir(dirPath: string): { files: string[]; dirs: string[] } {
		...
		for (const filePath of this.files.keys()) {
			...
			const rest =
				dirPath.length > 0 ? filePath.slice(prefix.length) : filePath;
...
function handleReadFile(
	...
	if (typeof args.path !== "string") {
		return errorPayload(
			"missing_argument",
			"read_file requires a 'path' string argument",
		);
	}
...
function handleSearchFiles(
	...
	if (typeof args.pattern !== "string" || args.pattern.length === 0) {
		return errorPayload(
			"missing_argument",
			"search_files requires a 'pattern' string argument",
		);
	}
...
function handleWriteFile(
	...
	if (typeof args.path !== "string") {
		return errorPayload(
			"missing_argument",
			"write_file requires a 'path' string argument",
		);
	}
	if (typeof args.content !== "string") {
		return errorPayload(
			"missing_argument",
			"write_file requires a 'content' string argument",
		);
	}
...
function validatePath(
	...
	if (typeof raw !== "string") {
		return {
			...
			error: errorPayload(
				"missing_argument",
				`${toolName} requires a 'path' string argument`,
			),
		};
	}
	try {
...
function handleGrep(
	...
	if (typeof args.pattern !== "string" || args.pattern.length === 0) {
		return errorPayload(
			"missing_argument",
			"grep requires a 'pattern' string argument",
		);
	}
	...
	const exactMatch = basePath.length > 0 ? files.get(basePath) : undefined;
	if (exactMatch !== undefined) {
		// Exact file path — search only this file
		candidateFiles = [[basePath, exactMatch]];
	} else {
...
function handleFind(
	...
	if (typeof args.pattern !== "string" || args.pattern.length === 0) {
		return errorPayload(
			"missing_argument",
			"find requires a 'pattern' string argument",
		);
	}
...
function handleBash(
	...
	if (typeof args.command !== "string" || args.command.length === 0) {
		return errorPayload(
			"missing_argument",
			"bash requires a 'command' string argument",
		);
	}
	...
	if (pkg) {
		...
		try {
			...
			if (parsed.scripts?.test) {
				...
				const src =
					workspace.readFile("src/index.ts") ??
					workspace.readFile("src/main.ts");
				if (src?.includes("export function add")) {
